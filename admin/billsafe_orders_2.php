...
*/
	...
	require_once (DIR_FS_CATALOG.'includes/classes/billsafe_2/billsafe_2.php');
	require (DIR_FS_CATALOG.'includes/classes/billsafe_2/ini.php');

	$bs = new Billsafe_Sdk(DIR_FS_CATALOG.'includes/classes/billsafe_2/ini.php');
	...
	if (MODULE_PAYMENT_BILLSAFE_2_LOG == 'True') {
		if (MODULE_PAYMENT_BILLSAFE_2_LOG_TYPE == 'Echo') {
			require_once DIR_FS_CATALOG.'includes/classes/billsafe_2/LoggerEcho.php';
			$bs->setLogger(new Billsafe_LoggerEcho());
		} elseif (MODULE_PAYMENT_BILLSAFE_2_LOG_TYPE == 'Mail') {
			require_once DIR_FS_CATALOG.'includes/classes/billsafe_2/LoggerMail.php';
			$bs->setLogger(new Billsafe_LoggerMail(MODULE_PAYMENT_BILLSAFE_2_LOG_ADDR));
		} elseif (MODULE_PAYMENT_BILLSAFE_2_LOG_TYPE == 'File') {
			require_once DIR_FS_CATALOG.'includes/classes/billsafe_2/LoggerFile.php';
	...
	$orders_query = xtc_db_query('SELECT id, transactionid, billsafeStatus FROM billsafe_orders_2 WHERE orderid = "'.$order_id.'"');
	...
	if (isset($_POST['reportShipmentFull'])) {
		...
		if ($responseShipment->ack == 'OK' && $full_shipment_count == 0) {
			...
			xtc_db_query('UPDATE billsafe_orders_details_2 SET shipped = 1 WHERE ordernumber = "'.$order_id.'"');
			$messageBox = 'SUCCESS';
		...
		$queryLeftArticles = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND articletype = "goods" AND shipped = 0 AND storno = 0');
		$countArticles = xtc_db_num_rows($queryLeftArticles);
		$counter = 0;
		if ($countArticles != 0) {
			foreach ($orderArticles as $key => $value) {
				$queryOrders = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE articlenumber = "'.$key.'" AND ordernumber = "'.$order_id.'"');
			...
			$queryLeftArticles = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND shipped = 0 AND storno = 0 GROUP BY id');
			while ($leftArticles = xtc_db_fetch_array($queryLeftArticles)) {
				xtc_db_query('UPDATE billsafe_orders_details_2 SET shipped = 1 WHERE id = "'.$leftArticles['id'].'"');
				xtc_db_query('INSERT INTO billsafe_transactions_2 (ordernumber, articlenumber, transactionmethod, bsorder_id, bsordersdetails_id, date) VALUES ("'.$order_id.'", "'.$leftArticles['articlenumber'].'", "reportShipmentPart", "'.$billsafe_orders['id'].'", "'.$leftArticles['id'].'", CURRENT_TIMESTAMP);');
			}
		}
		...
		if (($total + $amount) >= 0) {
			...
			xtc_db_query('INSERT INTO billsafe_orders_details_2 (ordernumber, articletype, articlenumber, articlename, articleprice, articletax, bsorders_id) VALUES ("'.$order_id.'", "voucher", "backend-voucher-'.($countVoucher + 1).'", "'.MODULE_PAYMENT_BILLSAFE_2_UPDATEARTICLELISTVOUCHER.'-'.($countVoucher + 1).'", "'.$amount.'", "'.$voucher_tax_rate.'", "'.$billsafe_orders['id'].'");');
			get_bsod($order_id);
			$total = 0;
			$totalTax = 0;
			getTotalsOrderProducts($order_id);
			...
			$response = $bs->callMethod('updateArticleList', $params);
			if ($response->ack == "OK") {
				xtc_db_query('INSERT INTO billsafe_transactions_2 (id, ordernumber, articlenumber, transactionmethod, bsorder_id, bsordersdetails_id, date) VALUES (NULL, "'.$order_id.'", "backend-voucher-'.($countVoucher + 1).'", "updateArticleListVoucher", "'.$billsafe_orders['id'].'", "'.$addVoucher['id'].'", CURRENT_TIMESTAMP);');
				$messageBox = 'SUCCESS';
		...
		if ($response->ack == 'OK') {
			...
			xtc_db_query('UPDATE billsafe_orders_details_2 SET retoure = 1 WHERE ordernumber = "'.$order_id.'"');
			$messageBox = 'SUCCESS';
		...
		if ($response->ack == 'OK') {
			...
			xtc_db_query('UPDATE billsafe_orders_details_2 SET storno = 1 WHERE ordernumber = "'.$order_id.'"');
			$messageBox = 'SUCCESS';
		...
		if ($response->ack == 'OK') {
			...
			xtc_db_query('UPDATE billsafe_orders_details_2 SET pause = "'.$pauseDays.'" WHERE ordernumber = "'.$order_id.'" AND storno = "0" AND retoure = "0"');
			$messageBox = 'SUCCESS';
	...
	function insDB_trans($order, $order_id, $action, $bs_orders_id, $sql_order_id) {
		...
		while ($order=xtc_db_fetch_array($queryOrders)) {
			xtc_db_query('INSERT INTO billsafe_transactions_2 (id, ordernumber, articlenumber, transactionmethod, bsorder_id, bsordersdetails_id, date) VALUES (NULL, "'.$order_id.'", "'.$order['articlenumber'].'", "'.$action.'", "'.$bs_orders_id.'", "'.$order['id'].'", CURRENT_TIMESTAMP);');
		}
	}
	...
	function updDB_art_part($type, $orderIDArr) {
		...
		for ($i = 0; $i < count($orderIDArr); $i++) {
			...
			$querybsod = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE id = "'.$sOrderArtId.'"');
			...
			xtc_db_query('UPDATE billsafe_orders_details_2 SET '.$type.' = 1 WHERE id = "'.$sOrderArtId.'"');
		}
	}
	...
	function getTotalsOrderProducts($order_id) {
		...
		$orderQuery = xtc_db_query('SELECT * FROM billsafe_orders_details_2 bsod WHERE ordernumber = "'.$order_id.'"');
	...
	function updDB_ot($action, $order_id) {
		global $orderIDArr;
		$queryLeftArticles = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND articletype = "goods" AND (shipped = 0 AND storno = 0) OR (shipped = 1 AND retoure = 0)');
		$countArticles = xtc_db_num_rows($queryLeftArticles);
		if ($countArticles == 0) {
			$queryLeftArticles = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND (shipped = 0 AND storno = 0) OR (shipped = 1 AND retoure = 0) GROUP BY id');
			while ($leftArticles = xtc_db_fetch_array($queryLeftArticles)) {
				xtc_db_query('UPDATE billsafe_orders_details_2 SET '.$action.' = 1 WHERE id = "'.$leftArticles['id'].'"');
			}
		}
	}
	...
	function get_bsod($order_id) {
		global $articleList;
		$counter = 0;
		$querybsod = xtc_db_query('SELECT COUNT(articlenumber) AS quantity, articlenumber FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" GROUP BY articlenumber');
		while ($bsod = xtc_db_fetch_array($querybsod)) {
			$queryOrders = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE articlenumber = "'.$bsod['articlenumber'].'" AND ordernumber = "'.$order_id.'"');
			$order = xtc_db_fetch_array($queryOrders);
			$articleList[$counter]['number'] = $order['articlenumber'];
			$articleList[$counter]['name'] = $order['articlename'];
	...
	function getSSRarr($order_id, $artno) {
		$shippedArray = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS shipped FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND shipped = 1 AND articlenumber = "'.$artno.'"'));
		$ssrarr['shipped'] = $shippedArray['shipped'];
		$stornoArray = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS storno FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND storno = 1 AND articlenumber = "'.$artno.'"'));
		$ssrarr['storno'] = $stornoArray['storno'];
		$retoureArray = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS retoure FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND retoure = 1 AND articlenumber = "'.$artno.'"'));
	...
	function getTotal0($order_id, $action) {
		$orderIDArr = array();
		$orderVoucher = xtc_db_query('SELECT * FROM billsafe_vouchers_2 bv, billsafe_orders_details_2 bsod WHERE bsod.ordernumber = "'.$order_id.'" AND bsod.id = bv.id AND bsod.articlenumber LIKE "backend-voucher-%" AND bsod.storno = 0 AND retoure = 0');
		while ($orderData = xtc_db_fetch_array($orderVoucher)) {
			xtc_db_query('UPDATE billsafe_orders_details_2 SET '.$action.' = 1 WHERE id = "'.$orderData['id'].'"');
	...
	function getBillSAFEresp($response, $defmess, $method, $action, $orderIDArr, $order_id, $billsafe_orders_id) {
		...
		if ($response->ack == 'OK') {
			foreach ($orderIDArr as $orderID) {
				$values = array();
				$resultArticle = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE id = "'.$orderID.'"');
				$values = xtc_db_fetch_array($resultArticle);
				xtc_db_query('INSERT INTO billsafe_transactions_2 (id, ordernumber, articlenumber, transactionmethod, bsorder_id, bsordersdetails_id, date) VALUES (NULL, "'.$order_id.'", "'.$values['articlenumber'].'", "'.$method.'", "'.$billsafe_orders_id.'", "'.$values['id'].'", CURRENT_TIMESTAMP);');
			}
			$messageBox = 'SUCCESS';
			$message = $defmess;
		} else {
			...
			for ($i = 0; $i < count($orderIDArr); $i++) {
				$sOrderArtId = $orderIDArr[$i];
				xtc_db_query('UPDATE billsafe_orders_details_2 SET '.$action.' = 0 WHERE id = "'.$sOrderArtId.'"');
			}
		}
	}
...
<td class="pageHeading" align="right"><?php echo '<a class="button" href="'.xtc_href_link(FILENAME_ORDERS, xtc_get_all_get_params(array('action'))).'">'.BUTTON_BACK.'</a>'; ?></td></tr>
...
<td class="main"><?php
	$customer = xtc_db_fetch_array(xtc_db_query('SELECT * FROM billsafe_orders_user_2 WHERE bsorders_id  ="'.$billsafe_orders['id'].'"'));
	$country_query = xtc_db_query('SELECT countries_name FROM countries WHERE countries_iso_code_2 = "'.$customer['country'].'"');
...
<td class="dataTableHeadingContent" align="center"></td>
	...
	$countOrder = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS quantity FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'"'));
	$countUsed = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS quantity FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND (shipped = 1 OR storno = 1)'));
	$countShipped = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS quantity FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND shipped = 1'));
	$countRetoure = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS quantity FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND (shipped = 1 AND retoure = 1)'));
	$countNotRetoure = xtc_db_fetch_array(xtc_db_query('SELECT COUNT(articlenumber) AS quantity FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'" AND (shipped = 1 AND retoure = 0)'));
	$orderQuery = xtc_db_query('SELECT * FROM billsafe_orders_details_2 WHERE ordernumber = "'.$order_id.'"');
	while ($orderData = xtc_db_fetch_array($orderQuery)) {
...
<br /><?php
	...
	$prod_history_query = xtc_db_query('SELECT bd.articlenumber, bd.articletype, bsod.bsordersdetails_id, bd.articlename, bsod.transactionmethod, bsod.date, bd.retoure, bd.pause FROM billsafe_transactions_2 bsod, billsafe_orders_details_2 bd WHERE bsod.ordernumber = "'.$order_id.'" AND bd.ordernumber = "'.$order_id.'" AND bsod.bsordersdetails_id = bd.id');
...
<a class="button" href="https://client.billsafe.de" target="_blank"><?php echo MODULE_PAYMENT_BILLSAFE_2_MP; ?></td>
<td><table border="0" cellspacing="0" cellpadding="2"><tr class="dataTableHeadingRow">
