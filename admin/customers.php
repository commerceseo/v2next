...
if ($_GET['action']) {
    switch ($_GET['action']) {
        case 'new_order' :
            $customers1 = xtc_db_fetch_array(xtc_db_query("SELECT * FROM " . TABLE_CUSTOMERS . " WHERE customers_id = '" . (int) $_GET['cID'] . "';"));
...
<table class="outerTable" cellpadding="0" cellspacing="0">
            ...
            <table class="table_pageHeading" border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td align="right">
                        ...
                        <?php echo HEADING_TITLE_SEARCH . ' ' . xtc_draw_input_field('search'); ?>&nbsp;
                        <?php echo '<a class="button" href="' . xtc_href_link(FILENAME_CREATE_ACCOUNT) . '">' . BUTTON_CREATE_ACCOUNT . '</a>'; ?>
                        </form>
                    </td>
                </tr>
            </table>
    ...
    if (($_GET['searchcid']) && (xtc_not_null($_GET['searchcid']))) {
        $keywords = xtc_db_input(xtc_db_prepare_input($_GET['searchcid']));
        $search_word = "AND (c.customers_id = '" . $keywords . "')";
    }
