 ...
 *	@copyright	Authors
 *            
 *  contains Gambio-modifications 2014-10-10
 */
...
class Filemanager {
	...
	public function __construct($extraConfig = '') {
		...
		// BOF Gambio 
		ob_start();
		include("../../scripts/filemanager.config.js.php");
		...
		// BOF Gambio
		$this->doc_root = substr(DIR_FS_CATALOG, 0, strlen(DIR_FS_CATALOG) - strlen(DIR_WS_CATALOG)+1);
		// EOF Gambio
		...
		if(!$handle = opendir($current_path)) {
			...
			while (false !== ($file = readdir($handle))) {
				if($file != "." && $file != "..") {
					...
					if(!preg_match('/(?:[\xC2-\xDF][\x80-\xBF]|\xE0[\xA0-\xBF][\x80-\xBF]|[\xE1-\xEC\xEE\xEF][\x80-\xBF]{2}|\xED[\x80-\x9F][\x80-\xBF]|\xF0[\x90-\xBF][\x80-\xBF]{2}|[\xF1-\xF3][\x80-\xBF]{3}|\xF4[\x80-\x8F][\x80-\xBF]{2})+/xs', $file))
					{
	...
	private function get_file_info($path='', $thumbnail = false) {
		...
		// Gambio windows server open_basedir fix:
		if(is_dir(substr(DIR_FS_CATALOG, 0, strlen(DIR_WS_CATALOG) * -1) . $current_path)) {
...
private function getFullPath($path = '') {
	...
	// BOF Gambio
	// Fix: rawurlencode to encode spaces/%20 in filename
	...
	if($file_exists !== true)
	{
		$full_path = utf8_decode($file_exists);
	}
	// EOF Gambio
