...
class order {
    ...
    function query($order_id) {
        ...
        $this->info = array('order_id' => $order['order_id'],
        ...
        while ($orders_products = xtc_db_fetch_array($orders_products_query)) {
            $this->products[$index] = array('qty' => $orders_products['products_quantity'],
                ...
                'products_shipping_time' => $orders_products['products_shipping_time'],
    ...
    function getOrderData($oID) {
        ...
        $order_query = xtc_db_query("SELECT products_id,
                             orders_products_id,
                             products_model,
                             products_name,
                             final_price,
    ...
    function getTotalData($oID) {
        ...
        $order_total_query = xtc_db_query("SELECT title,
                                   text,
                                   class,
                                   value,
                                   sort_order
	...
	function cart($customer_id) {
        ...
        $customer_address = xtc_db_fetch_array(xtc_db_query("SELECT c.payment_unallowed,c.shipping_unallowed,c.customers_firstname,c.customers_cid, c.customers_gender,c.customers_lastname, c.customers_telephone, c.customers_email_address, c.customers_default_address_id, ab.entry_company, ab.entry_street_address, ab.entry_suburb, ab.entry_postcode, ab.entry_city, ab.entry_zone_id, z.zone_name, co.countries_id, co.countries_name, co.countries_iso_code_2, co.countries_iso_code_3, co.address_format_id, ab.entry_state from " . TABLE_CUSTOMERS . " c, " . TABLE_ADDRESS_BOOK . " ab left join " . TABLE_ZONES . " z on (ab.entry_zone_id = z.zone_id) left join " . TABLE_COUNTRIES . " co on (ab.entry_country_id = co.countries_id) where c.customers_id = '" . $customer_id . "' and ab.customers_id = '" . $customer_id . "' and c.customers_default_address_id = ab.address_book_id;"));
        ...
        $tax_address = xtc_db_fetch_array(xtc_db_query("SELECT ab.entry_country_id, ab.entry_zone_id from " . TABLE_ADDRESS_BOOK . " ab left join " . TABLE_ZONES . " z on (ab.entry_zone_id = z.zone_id) where ab.customers_id = '" . $customer_id . "' and ab.address_book_id = '" . $customer_address['customers_default_address_id'] . "';"));
