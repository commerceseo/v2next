...
/* -----------------------------------------------------------------
 * 	$Id: import.php 797 2014-01-13 17:47:22Z akausch $
    ...
    function insertProduct(& $dataArray, $mode = 'insert', $touchCat = false) {
        ...
        for ($i = 0; $i < count($this->Groups) - 1; $i++) {
            ...
            if (isset($dataArray['p_priceNoTax.' . $this->Groups[$i + 1]['id']]) && $dataArray['p_priceNoTax.' . $this->Groups[$i + 1]['id']] > 0) {
                $truncate_query = xtc_db_query("DELETE FROM " . TABLE_PERSONAL_OFFERS_BY . $this->Groups[$i + 1]['id'] . " WHERE products_id='" . $prod_data['products_id'] . "'");
...
class ordersExport {
    ...
    function getOrders() {
        $sql = "SELECT 
				...
				FROM 
					orders AS o
				LEFT JOIN orders_status AS os ON (o.orders_status = os.orders_status_id AND os.language_id = 2)
...
//Alles ausgeben
        ...
        $data = "Bestellung Nr.;Bestelldatum;Rechnungs-Nr.;Rechnungs-Datum;Kunden-Nr.;Kundenname;Land;UST-ID;Bezahlart;Status;Summe Netto;Versand Netto;Nachnahme Netto;UST.;Rechnungsbetrag Brutto;E-Mail\n";
...
//Ausgabe Summe Netto
            ...
            $exportemail = $export_data['customers_email_address'];
