...
class haendlerbund_importer {
            ...
            foreach ($agb_service as $agb_array => $key) {
                ...
                if ($api_config == 1) {
                    if (!preg_match("/DOCUMENT_NOT_AVAILABLE/i", $result) && !preg_match("/SHOP_NOT_FOUND/i", $result)) {
                        ...
                        mysql_query("UPDATE configuration SET configuration_value='1' WHERE configuration_key='" . $agb . "_hbon'");
                    } else {
                        mysql_query("UPDATE configuration SET configuration_value='0' WHERE configuration_key='" . $agb . "_hbon'");
                    }
                }
    ...
    function saveConfiguration() {
        foreach ($_POST as $configuration_key => $configuration_value) {
            if (preg_match("/haendlerbund/", $configuration_key)) {

                $sql = mysql_query("SELECT * FROM configuration WHERE configuration_key='" . $configuration_key . "' LIMIT 1");
                if (mysql_num_rows($sql))
                    mysql_query("UPDATE configuration SET configuration_value='" . $configuration_value . "' WHERE configuration_key='" . $configuration_key . "' LIMIT 1");
            }
        }
    }
    ...
    function updateContent($content_id, $content_text) {
     // if(strtolower($_SESSION['language_charset']) != "utf-8") $content_text = utf8_decode($content_text);
	 // echo $content_text;die;
	 $content_text = utf8_encode($content_text);
        $return = mysql_query("UPDATE content_manager SET content_text='" . $content_text . "' WHERE content_id=" . $content_id . " LIMIT 1");
        if (!mysql_error()) {
            $return = 1;
        }
        return $return;
...
<input type="hidden" name="config_save" value="1" />
        <div class="widget">
          <div class="wizard swMain">
            <ul>
              <li><a href="#step-1" class="bordLeft wFirst">
                <h5 class="stepDesc iComputer">Sicherheitsschl&uuml;ssel</h5>
              </a></li>
              <li><a href="#step-2" class="bordRight">
                <h5 class="stepDesc iSpeech">Inhaltszuweisung</h5>
              </a></li>
            </ul>
            <div id="step-1">
            ...
            <div id="step-2" style="height:350px">
              <p> Bitte weisen Sie jedem Rechtstext einen passenden Inhalt aus dem Content-Manager zu. <br />
<b>Achten Sie darauf, dass jeder Rechtstext einen unterschiedlichen Content zugewiesen ist.</b></p>';

            $return .= '<strong id="text"><center><br /><br />

<img src="includes/haendlerbund/images/loading.gif" /><br />
<br />
Einen Moment bitte, es wird geladen!</center></strong>';
    ...
    <div class="title_hb">
      ...
      <h5 class="title_hb">Rechtstexte importieren</h5></div>';
	...
	<input type="hidden" name="agb_import" value="1" />
        <div style="width:731px; height:300px; background-image:url(includes/haendlerbund/images/import_picture.jpg); background-position:top;"><div style="padding:10px; padding-top:50px;">
        ...
        <div style="line-height:125%;">';
            ...
            if ($_SESSION["import_message"] != "") {

                $return .= '<img src="includes/haendlerbund/images/import_ok_btn.png" width="217" height="52">';
                $_SESSION["import_message"] = "";
            } else {

                $return .= '<img src="includes/haendlerbund/images/pfleil_blue.png" width="217" height="52"><span style="color:#1E4565; font-weight:bold;"><br>
          WICHTIG!</span>  <br>
...
<input type="submit" name="import" value="" style="background-image:url(includes/haendlerbund/images/import_btn.png); width:196px; height:26px; border:0;" />';
  ...
  <a href="?start_konfig=1"><img src="includes/haendlerbund/images/konfig_btn.png" width="175" height="23" border="0"></a>
    ...
    function rt_gen() {
        ...
        $sql_check = mysql_query("SELECT configuration_key  FROM configuration WHERE configuration_key LIKE '%_hbon' AND configuration_value = 1 ORDER BY  configuration_id");
        while ($agb = mysql_fetch_array($sql_check)) {
    ...
    function getContentIDSelect($label, $name, $id, $required = true, $lang = 1) {
        ...
        $sql = mysql_query("SELECT content_id,content_title FROM content_manager WHERE languages_id=" . $_SESSION["languages_id"] . " ORDER BY content_title");
