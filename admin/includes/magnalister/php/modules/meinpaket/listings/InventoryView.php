...
class InventoryView {
	...
	public function __construct($settings = array()) {
		...
		if (isset($_POST['refreshStock'])) {
			...
			require_once (DIR_MAGNALISTER_MODULES.'meinpaket/crons/MeinpaketSyncInventory.php');
			$asi = new MeinpaketSyncInventory($this->magnasession['mpID'], 'meinpaket');
			...
			switch ($_POST['action']) {
				case 'delete': {
					...
					if ($result['STATUS'] == 'SUCCESS') {
						...
						magnaMeinpaketProcessCheckinResult($result, $this->magnasession['mpID']);
						$this->postDelete();
					}
					break;
				}
		...
		$html = '
			<table'.(($id != '') ? ' id="'.$id.'"' : '').' class="datagrid">
				<thead class="small"><tr>
					...
					<td>'.ML_MEINPAKET_LABEL_MEINPAKETID.' '.$this->sortByType('meinpaketid').'</td>
