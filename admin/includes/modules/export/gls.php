<?php
/*-----------------------------------------------------------------
* 	$Id: gls.php 615 2013-09-19 16:41:05Z akausch $
...
class gls {
	...
	function display() {
		...
		xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=gls')) .'</div>');
	...
	function install() {
		...
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_GLS_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
	}
