<?php
/* -----------------------------------------------------------------
 * 	$Id: module_newsletter_products.php 420 2013-06-19 18:04:39Z akausch $
...
switch ($_GET['action']) {
    case 'delete':
        ...
        if (isset($_GET['acid']) && is_numeric($_GET['acid'])) {
            xtc_db_query("DELETE FROM " . TABLE_NEWSLETTER_PRODUCT_LIST . " where id = '" . (int) $_GET['acid'] . "'");
            xtc_db_query("DELETE FROM " . TABLE_NEWSLETTER_PRODUCTS . " where accessories_id = '" . (int) $_GET['acid'] . "'");

            xtc_redirect(xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS));
        }
        break;
    ...
    case 'new':
        ...
        if (isset($_POST['products_list'])) {
            $exist_id_query = xtc_db_query("select id from " . TABLE_NEWSLETTER_PRODUCT_LIST . " where list_name = '" . $_POST['products_list'] . "'");
            ...
            if ($exist_id == 0) {
                ...
                xtc_db_perform(TABLE_NEWSLETTER_PRODUCT_LIST, $product_list_array);
                $select_id_query = xtc_db_query("select id from " . TABLE_NEWSLETTER_PRODUCT_LIST . " 
												 where list_name = '" . $_POST['products_list'] . "'");
                $select_id = xtc_db_fetch_array($select_id_query);

                xtc_redirect(xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=new&aid=' . (int) $select_id['id'] . '&name=' . $_POST['products_list']));
            } else {
                xtc_redirect(xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=edit&acid=' . (int) $exist['id']));
                break;
            }
        }
        break;
    ...
    case 'edit':
        ...
        if ($_POST['acp']) {
            ...
            if ($anz > 0) {
                for ($i = 0; $i < $anz; $i++) {
                    xtc_db_query("DELETE FROM " . TABLE_NEWSLETTER_PRODUCTS . " where id = '" . $_POST['acp'][$i] . "'");
                }
                xtc_redirect(xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=edit&acid=' . (int) $_GET['acid']));
                break;
            }
        }
        ...
        if ($_POST['accessories']) {
            ...
            if ($n > 0) {
                ...
                for ($i = 0; $i < $n; $i++) {
                    ...
                    xtc_db_perform(TABLE_NEWSLETTER_PRODUCTS, $accessories_product_array);
                }
                xtc_redirect(xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=edit&acid=' . (int) $_GET['acid']));
                break;
            }
...
<table class="outerTable" cellpadding="0" cellspacing="0">
    <tr>
        <td class="boxCenter" width="100%" valign="top">
            ...
            <table border="0" cellspacing="5" cellpadding="5">
                <tr>
                    <td><a class="button" href="<?php echo xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS); ?>"><?php echo LIST_OVERVIEW; ?></a></td>
                    <td><a class="button" href="<?php echo xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=new') ?>"><?php echo LIST_NEW; ?></a></td>
                </tr>  
            </table>
            <br>
            ...
            <?php
            if ($_GET['action'] == 'new') {
                echo xtc_draw_form('search', FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=new', 'post', '');
                ?>
                <table border="0" cellspacing="1" cellpadding="2">
                    <tr>
                        ...
                        <?php
                        if ($_GET['name']) {
                            $select_list_name = xtc_db_query("SELECT list_name FROM " . TABLE_NEWSLETTER_PRODUCT_LIST . "
                ...
                <?php
                if ($_GET['name']) {
                    echo xtc_draw_form('search', FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=edit&acid=' . (int) $_GET['aid'], 'post', '');
                    ?>
                    <table border="0" cellspacing="1" cellpadding="2">
                        <tr>
                ...
                echo xtc_draw_form('acces', FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=edit&acid=' . (int) $_GET['acid'], 'post', '');
    ...
    <?php
    if (isset($_GET['acid']) && is_numeric($_GET['acid'])) {
        echo xtc_draw_form('search', FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=edit&acid=' . (int) $_GET['acid'], 'post', '');
        ?>
                                <fieldset>
                ...
                <table width="100%" class="dataTable" cellspacing="0" cellpadding="0">
                    ...
                    <?php
                    $head_product_query = xtc_db_query("SELECT id, list_name FROM " . TABLE_NEWSLETTER_PRODUCT_LIST . " ORDER BY id ASC");
                    $rows = 1;
                    while ($head_product = xtc_db_fetch_array($head_product_query)) {
                        $count_accessories = xtc_db_query("SELECT COUNT(ap.id) AS total 
									   FROM " . TABLE_NEWSLETTER_PRODUCTS . " ap, " . TABLE_NEWSLETTER_PRODUCT_LIST . " a 
                ...
                <td class="access_step_cc">
                    <a href="<?php echo xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=edit&acid=' . (int) $head_product['id']) ?>">
                        <img src="images/icon_edit.gif" alt="" title="<?php echo ACTION_EDIT; ?>" />
                    </a>&nbsp;
                    <a href="<?php echo xtc_href_link(FILENAME_MODULE_NEWSLETTER_PRODUCTS, 'action=delete&acid=' . (int) $head_product['id']) ?>">
