<?php
/* -----------------------------------------------------------------
 * 	$Id: orders_edit.php 872 2014-03-21 14:46:30Z akausch $
...
if ($action == 'product_option_ins') {
    ...
    $lang = xtc_db_fetch_array(xtc_db_query("SELECT languages_id FROM " . TABLE_LANGUAGES . " WHERE directory = '" . $order->info['language'] . "';"));
    $status = get_customers_taxprice_status();
    $products_attributes = xtc_db_fetch_array(xtc_db_query("SELECT * FROM " . TABLE_PRODUCTS_ATTRIBUTES . " WHERE products_attributes_id = '" . (int) $_POST['aID'] . "';"));
    $products_options = xtc_db_fetch_array(xtc_db_query("SELECT products_options_name FROM " . TABLE_PRODUCTS_OPTIONS . " WHERE products_options_id = '" . (int) $products_attributes['options_id'] . "' AND language_id = '" . (int) $lang['languages_id'] . "';"));
    $products_options_values = xtc_db_fetch_array(xtc_db_query("SELECT products_options_values_name FROM " . TABLE_PRODUCTS_OPTIONS_VALUES . " WHERE products_options_values_id = '" . (int) $products_attributes['options_values_id'] . "' AND language_id = '" . (int) $lang['languages_id'] . "';"));
    ...
    $sql_data_array = array('orders_id' => (int) ($_POST['oID']),
							...
							'products_attributes_id' => xtc_db_prepare_input($products_attributes['products_attributes_id']),
							'sortorder' => xtc_db_prepare_input($products_attributes['sortorder']),
	...
	//Attribute QTY aktualisieren
	if (isset($products_attributes['products_attributes_id'])) {
		xtc_db_query("UPDATE " . TABLE_PRODUCTS_ATTRIBUTES . " SET attributes_stock = attributes_stock - " . $products['products_quantity'] . " WHERE products_attributes_id = " . $products_attributes['products_attributes_id']);
	}
    $products_a_query = xtc_db_query("SELECT options_values_price, price_prefix FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " WHERE orders_products_id = '" . (int) $_POST['opID'] . "'");
...
if ($action == 'product_delete') {
	$old_attribut_id = xtc_db_fetch_array(xtc_db_query("SELECT products_attributes_id from " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " where orders_products_id = '" . (int) ($_POST['opID']) . "'"));
	if ($old_attribut_id['products_attributes_id'] != '') {
		xtc_db_query("UPDATE " . TABLE_PRODUCTS_ATTRIBUTES . " SET attributes_stock = attributes_stock + " . xtc_db_input($_POST['del_qty']) . " WHERE products_attributes_id = " . $old_attribut_id['products_attributes_id']);
	}
...
if ($action == 'product_option_delete') {
	...
	$old_attribut_id = xtc_db_fetch_array(xtc_db_query("SELECT products_attributes_id from " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " where orders_products_attributes_id = '" . (int) ($_POST['opAID']) . "'"));
	if ($old_attribut_id['products_attributes_id'] != '') {
		xtc_db_query("UPDATE " . TABLE_PRODUCTS_ATTRIBUTES . " SET attributes_stock = attributes_stock + " . $products['products_quantity'] . " WHERE products_attributes_id = " . $old_attribut_id['products_attributes_id']);
    }
