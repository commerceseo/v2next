...
if (($actionSave) && empty($messageBox)) {
    ...
    $details['transactionID'] = $cabOrderDetails['transactionID'];
    $details['externalID'] = $externalID;
    $items[1]['itemType'] = 'item1Text';
    $items[1]['textItemDescription'] = $itemDesc1;

    $requestResult = $cabApi->refundRequest($authentication, $details, $items);
    ...
    if ($requestValues['transaction']['transactionStatus'] == 'SUCCESS' OR $requestValues['transaction']['transactionStatus'] == 'IN_PROGRESS') {
        xtc_db_query("UPDATE picab_orders SET refunded =  '" . $refunded . "' WHERE shopOrderID ='" . $shopOrderID . "'");
        $messageBox = 'SUCCESS';

        $transactionID = $requestValues['transaction']['transactionID'];
        $transactionStatus = $requestValues['transaction']['transactionStatus'];
        xtc_db_query("INSERT INTO picab_transactions (id,shopOrderID,transactionID,externalID,transactionType,description,amount,currency,paid,status,created,modified) VALUES (NULL,'" . $shopOrderID . "','" . $transactionID . "','" . $externalID . "','refund','" . $itemDesc1 . "'," . $postAmount . ",'" . $cabOrderDetails['currency'] . "',1,'" . $transactionStatus . "',NOW(),NOW())");
    } else {
        $intPlaces = $intPlaces;
        $decPlaces = $decPlaces;
        $refunded = (float) $cabOrderDetails['refunded'];
                    ...
                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                        ...
                        <tr>
                            ...
                            if ($messageBox == 'SUCCESS') {
                                $showMessage = $cabXtcApi->showMessageSuccess(CLICKANDBUY_ORDER_REFUND_SUCCESSFUL . ' (' . CLICKANDBUY_ORDER_DETAILS_CLICKANDBUY_STATUS . ': ' . $transactionStatus . ').');
                                echo $showMessage;
                            } elseif ($messageBox == 'ERROR') {
                                $showMessage = $cabXtcApi->showMessageError(CLICKANDBUY_ORDER_REFUND_ERROR . ' ' . $errorDescription);
                                echo $showMessage;
                            }
                            ?>
                                <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                            ...
                                                            <table cellspacing="0" cellpadding="5" border="0" width="400">
                                                                <tbody>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td class="infoBoxContent"><input class="submit" type="submit" value="<?php echo CLICKANDBUY_ORDER_REFUND_REFUND_NOW; ?>" name="refund"></td>
                                                                    </tr>
                                                                </tbody>
                                                            </table>
                                                        </td>
