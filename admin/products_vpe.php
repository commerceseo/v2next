...
switch ($_GET['action']) {
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            ...
            if ($_GET['action'] == 'insert') {
                if (!xtc_not_null($products_vpe_id)) {
                    $next_id = xtc_db_fetch_array(xtc_db_query("SELECT max(products_vpe_id) AS products_vpe_id FROM " . TABLE_PRODUCTS_VPE . ";"));
    ...
    case 'deleteconfirm':
        ...
        $products_vpe = xtc_db_fetch_array(xtc_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'DEFAULT_PRODUCTS_VPE_ID';"));
	...
	<tr>
				...
				<tr>
					<td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
							...
							if (substr($_GET['action'], 0, 3) != 'new') {
								?>
								<tr>
									<td colspan="2" align="right"><?php echo '<a class="button" href="' . xtc_href_link(FILENAME_PRODUCTS_VPE, 'page=' . $_GET['page'] . '&action=new') . '">' . BUTTON_INSERT . '</a>'; ?></td>
								</tr>
								<?php
							}
							?>
		...
		switch ($_GET['action']) {
			case 'new':
				...
				$languages = xtc_get_languages();
				for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
					$products_vpe_inputs_string .= '<br />' . xtc_image(DIR_WS_LANGUAGES . $languages[$i]['directory'] . '/' . $languages[$i]['image']) . '&nbsp;' . xtc_draw_input_field('products_vpe_name[' . $languages[$i]['id'] . ']');
				}
				...
				$contents[] = array('align' => 'center', 'text' => '<br /><input type="submit" class="button" value="' . BUTTON_INSERT . '"/> <a class="button" href="' . xtc_href_link(FILENAME_PRODUCTS_VPE, 'page=' . $_GET['page']) . '">' . BUTTON_CANCEL . '</a>');
				break;
			...
			case 'edit':
				...
				$languages = xtc_get_languages();
				for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
					$products_vpe_inputs_string .= '<br />' . xtc_image(DIR_WS_LANGUAGES . $languages[$i]['directory'] . '/' . $languages[$i]['image']) . '&nbsp;' . xtc_draw_input_field('products_vpe_name[' . $languages[$i]['id'] . ']', xtc_get_products_vpe_name($oInfo->products_vpe_id, $languages[$i]['id']));
				}

				$contents[] = array('text' => '<br />' . TEXT_INFO_PRODUCTS_VPE_NAME . $products_vpe_inputs_string);
				if (DEFAULT_PRODUCTS_VPE_ID != $oInfo->products_vpe_id)
					$contents[] = array('text' => '<br />' . xtc_draw_checkbox_field('default') . ' ' . TEXT_SET_DEFAULT);
				$contents[] = array('align' => 'center', 'text' => '<br /><input type="submit" class="button" value="' . BUTTON_UPDATE . '"/> <a class="button" href="' . xtc_href_link(FILENAME_PRODUCTS_VPE, 'page=' . $_GET['page'] . '&oID=' . $oInfo->products_vpe_id) . '">' . BUTTON_CANCEL . '</a>');
				break;
			...
			case 'delete':
				...
				if ($remove_status)
					$contents[] = array('align' => 'center', 'text' => '<br /><input type="submit" class="button" value="' . BUTTON_DELETE . '"/> <a class="button" href="' . xtc_href_link(FILENAME_PRODUCTS_VPE, 'page=' . $_GET['page'] . '&oID=' . $oInfo->products_vpe_id) . '">' . BUTTON_CANCEL . '</a>');
				break;
			...
			default:
				if (is_object($oInfo)) {
					...
					$contents[] = array('align' => 'center', 'text' => '<a class="button" href="' . xtc_href_link(FILENAME_PRODUCTS_VPE, 'page=' . $_GET['page'] . '&oID=' . $oInfo->products_vpe_id . '&action=edit') . '">' . BUTTON_EDIT . '</a> <a class="button" href="' . xtc_href_link(FILENAME_PRODUCTS_VPE, 'page=' . $_GET['page'] . '&oID=' . $oInfo->products_vpe_id . '&action=delete') . '">' . BUTTON_DELETE . '</a>');

					$products_vpe_inputs_string = '';
					$languages = xtc_get_languages();
					for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
						$products_vpe_inputs_string .= '<br />' . xtc_image(DIR_WS_LANGUAGES . $languages[$i]['directory'] . '/' . $languages[$i]['image']) . '&nbsp;' . xtc_get_products_vpe_name($oInfo->products_vpe_id, $languages[$i]['id']);
					}
