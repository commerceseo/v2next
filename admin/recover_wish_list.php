<?php
/* -----------------------------------------------------------------
 * 	$Id: recover_wish_list.php 872 2014-03-21 14:46:30Z akausch $
...
if (isset($_GET['action']) && $_GET['action'] == 'complete') {
    ...
    if (RCS_DELETE_COMPLETED_ORDERS == 'true') {
        xtc_db_query("delete from " . TABLE_CUSTOMERS_WISHLIST . " where customers_id=" . $cID);
        xtc_db_query("delete from " . TABLE_CUSTOMERS_WISHLIST_ATTRIBUTES . " where customers_id=" . $cID);
...
if ($_GET['action'] == 'delete') {
    ...
    xtc_redirect(xtc_href_link(FILENAME_RECOVER_WISH_LIST, 'delete=1&customer_id=' . $_GET['customer_id'] . '&tdate=' . $_GET['tdate']));
}
    ...
    if (RCS_CHECK_SESSIONS == 'true') {
        if (STORE_SESSIONS == 'mysql') {
            ...
            while ($ses = xtc_db_fetch_array($sesquery)) {
                if (preg_match("/customer_id[^\"]*\"([0-9]*)\"/i", $ses['value'], $custval))
...
<?php if (count($_POST['custid']) > 0) { ?>
    ...
    foreach ($_POST['custid'] as $cid) {
        $quantity = array();
        $products_data = array();
        $quantityQuery = xtc_db_query("SELECT products_id pid, customers_basket_quantity qty FROM " . TABLE_CUSTOMERS_WISHLIST . " WHERE customers_id=" . $cid);
        ...
        $query1 = xtc_db_query("select cb.products_id pid,
                          ...
                          from      " . TABLE_CUSTOMERS_WISHLIST . " cb,
                                    " . TABLE_CUSTOMERS . " cus,
        ...
        for ($i = 0; $i < $knt; $i++) {
            ...
            if ($lastcid != $cid) {
              ...
              <tr>
                 <td colspan='6' align='right'><a class=\"button\" href=" . xtc_href_link(FILENAME_RECOVER_WISH_LIST, "action=delete&customer_id=" . $cid . "&tdate=" . $tdate) . ">" . BUTTON_DELETE . "</a></td>
              </tr>";
                    echo $cline;
                }
                $cline = "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" class=\"dataTable\">
						<tr> 
							<td class='dataTableContent' align='left' colspan='6' nowrap><a href='" . xtc_href_link(FILENAME_CUSTOMERS, 'search=' . $inrec['lname'], 'NONSSL') . "'>" . $inrec['fname'] . " " . $inrec['lname'] . "</a>" . $customer . "
							</td>
						</tr>";
                $tprice = 0;
            }
            ...
            if (RCS_SHOW_ATTRIBUTES == 'true') {
                $attribquery = xtc_db_query("select cba.products_id pid,
                                         ...
                                         from " . TABLE_CUSTOMERS_WISHLIST_ATTRIBUTES . " cba,
            ...
            $products_data[] = array(
                'QUANTITY' => $inrec['qty'],
                'NAME' => $inrec2['name'],
                'LINK' => xtc_catalog_href_link(FILENAME_CATALOG_PRODUCT_INFO, xtc_product_link($inrec['pid'])),
        ...
        $mail_data = cseo_get_mail_data('recover_cart_sales');
        ...
        if ($inrec['email'] != '') {
            xtc_php_mail($mail_data['EMAIL_ADDRESS'], $from_name, $inrec['email'], $custname, '', $mail_data['EMAIL_REPLAY_ADDRESS'], $from_name, '', '', $recover_cart_sales_subject, $html_mail, $txt_mail);
        }
    ...
    echo "<tr><td colspan=6 align='right'><a class=\"button\" href=" . xtc_href_link(FILENAME_RECOVER_WISH_LIST, "action=delete&customer_id=" . $cid . "&tdate=" . $tdate) . ">" . BUTTON_DELETE . "</a></td>  </tr>\n";
    ...
    $ndate = seadate($tdate);
    $query1 = xtc_db_query("SELECT customers_id, MAX(customers_basket_date_added) as last FROM " . TABLE_CUSTOMERS_WISHLIST . " WHERE customers_basket_date_added>='" . $ndate . "' " . $cust_sql . " GROUP BY customers_id ORDER BY last DESC, customers_id");
    ...
    while ($query1Res = xtc_db_fetch_array($query1)) {
        $cline .= '<table width="100%" cellspacing="0" cellpadding="0" class="dataTable">';
        $quantity = array();
        $quantityQuery = xtc_db_query("SELECT products_id pid, customers_basket_quantity qty FROM " . TABLE_CUSTOMERS_WISHLIST . " WHERE customers_id=" . $query1Res['customers_id']);
        while ($quantityResult = xtc_db_fetch_array($quantityQuery)) {
            $quantity[(int) $quantityResult['pid']] += $quantityResult['qty'];
        }
        ...
        while ($data = xtc_db_fetch_array($query2)) {
            ...
            if ($curcus != $inrec['cid']) {
                ...
                if ($curcus != "") {
                    ...
                    $ccquery = xtc_db_query('SELECT orders_id, orders_status
												        FROM ' . TABLE_ORDERS . '
            ...
            if ($inrec['qty'] != 0) {
                ...
                if ($i % 2 == 0)
                    ...
                    $f = '';
                $cline .= "<tr class=" . $f . ">
                    <td class='dataTableContent' align='left' vAlign='top' colspan='2' width='10%' nowrap>" . ($inrec['bdate'] < $ndate ? " x" : " &nbsp;") . "</td>
                    <td class='dataTableContent' align='left' vAlign='top' width='15%' nowrap>" . ($inrec2['model'] ? $inrec2['model'] : "&nbsp;") . "</td>
                    ...
                    <td class='dataTableContent' align='center' vAlign='top' width='5%' nowrap>" . $inrec['qty'] . "x</td>
        ...
        if ($final_line) {
            ...
            /* Beta        
              ...
              <tr>
              <td class='no_border' align='right'><a class=\"button\" href=" . xtc_href_link(FILENAME_RECOVER_WISH_LIST,"action=delete&customer_id=$curcus&tdate=$tdate") . ">" . BUTTON_DELETE  . "</a><a class=\"button\" href=" . xtc_href_link(FILENAME_RECOVER_WISH_LIST,"action=complete&customer_id=$curcus&tdate=$tdate") . ">" . BUTTON_COMPLETE  . "</a></td>
              </tr></table>\n";
             */
            if (!$skip) {
                echo $cline;
...
// echo '</table>';
    ...
    echo $cline;
    echo "<table width='100%'><tr><td><b>" . PSMSG . "</b><br>" . xtc_draw_textarea_field('message', 'soft', '80', '5')
    ?>
