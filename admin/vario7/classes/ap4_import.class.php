<?php
/**
 * @version $Id: ap4_import.class.php,v 1.1 2011-07-15 12:33:33 ag Exp $
 * @version $Revision: 1.1 $
 * @copyright Copyright (c) 2009 VARIO Software GmbH
 *
 * 22.05.2013 SV: AP4 = Kopie von AP4
 * 
 */
	class ap4_import extends vario_import {

		var $products_id, $quantity, $personal_offer;
		
		function ap4_import($exp_input){
			_debug($exp_input, 'START ap4_import -- Übergeben wurde der Datensatz $exp_input');
			...
			if (is_array($customers_status_ids)) {
				foreach ($customers_status_ids as $customer_status_id) {
					...
					// Welcher Preis aus der AP4 ist dieser Kundengruppe zugeordnet?
					...
					if ($preisgruppe > 1 and $preisgruppe <= 17 ) {
	    		    		...
	    		    		$this->personal_offer 	= $this->calculate_price_with_vario_preiseinheit(
														$this->getNettoPrice($this->get_exp_value('VK'.$preisgruppe), 
														$this->get_exp_value('MWSTSATZ')));

						_debug($this->personal_offer, '      ap4_import -- this->personal_offer');
