 ...
 *
 * $Id: helperFunctions.php 420 2013-06-19 18:04:39Z akausch $
 *
...
class HelperFunctions {
	...
	public static function escapeSql($string) {
		return (function_exists('mysql_real_escape_string') && mysql_ping()) ? mysql_real_escape_string($string) : mysql_escape_string($string);
	}
	...
	public static function replaceSpecialTextInHistoryComment($historyComments, $SofortLib_TransactionData, $transactionId){
		...
		$historyComments['buyer']  = preg_replace('#\{\{([a-zA-Z0-9_]+)\}\}#e', '$$1', $historyComments['buyer']);
	...
	public static function updateShopAdresses ($invoiceAddress, $shippingAddress, $orderId, $SofortLoggingObj = '') {
		...
		if (is_object($SofortLoggingObj)) {
			...
			} else if (function_exists('mysql_affected_rows')) {
	...
	public static function insertHistoryEntry($orderId, $status, $comment, $customerNotified = '', $time = '') {
		...
		if (!is_numeric($customerNotified)) {
			...
			if (HelperFunctions::isGambio() && $status != '-1') $customerNotified = '1';
		}
	...
	public static function getTransactionIdFromShop($orderId){
		$result = xtc_db_query('SELECT orders_ident_key FROM '.HelperFunctions::escapeSql(TABLE_ORDERS).' WHERE orders_id = "'.HelperFunctions::escapeSql($orderId).'"');
