...
/* -----------------------------------------------------------------
 * 	$Id: download.php 622 2013-09-22 17:25:29Z akausch $
...
xtc_db_query("UPDATE " . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " SET download_count = download_count-1, download_ip = '" . $_SERVER['REMOTE_ADDR'] . "', download_time = NOW() WHERE orders_products_download_id = '" . (int) $_GET['id'] . "'");
...
if (DOWNLOAD_BY_REDIRECT == 'true') {
    ...
    if (!($fd = fopen(DIR_FS_DOWNLOAD . $downloads['orders_products_filename'], 'rb')))
        return false;
    while (!feof($fd)) {
        echo fread($fd, 8192);
        ob_flush();
