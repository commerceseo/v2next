...
class Klarna
{
     ...
     * @var string
     */
    protected $VERSION = 'php:api:2.3.0:gambio_core:1';
    ...
    {
        $this->_logger = new FileLog('klarna-sdk', true);
    }
    ...
    public function addArticle(
        ...
        $this->_logger->write("adding article, qty = $qty, artNo = $artNo, title = $title, price = $price, vat = $vat, discount = $discount, flags = $flags\n");
        $this->_checkQty($qty);
        ...
        $this->_logger->write("final article data:\n".print_r($tmpArr, true)."\n");
    ...
    public function reserveAmount(
        ...
        $this->_logger->write("reserveAmount paramList:\n".print_r($paramList, true)."\n");
    ...
    public function fetchPClasses(
        ...
        $this->_logger->write("fetching pclasses for $country $language $currency\n");
    ...
    {
        $this->_logger->write("getting pclasses for country ".$this->_country);
        if (!($this->config instanceof ArrayAccess)) {
            throw new Klarna_IncompleteConfigurationException;
        }
        if (!$this->pclasses) {
    ...
    protected function xmlrpc_call($method, $array)
    {
        ...
        try {
            ...
            $this->_logger->write("sending xmlrpcmsg:\n".print_r($msg, true)."\n");
 ...
 */
require_once dirname(__FILE__).'/klarnaconfig.php';

/**
 * Include the {@link KlarnaPClass} class.
 */
require_once dirname(__FILE__).'/klarnapclass.php';

/**
 * Include the {@link KlarnaCalc} class.
 */
require_once dirname(__FILE__).'/klarnacalc.php';

/**
 * Include the {@link KlarnaAddr} class.
 */
require_once dirname(__FILE__).'/klarnaaddr.php';
