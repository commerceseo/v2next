...
/* -----------------------------------------------------------------
 * 	$Id: google_sitemap.php 934 2014-04-02 15:40:06Z akausch $
...
if ($_SESSION['customers_status']['customers_status_id'] == 0) {
	...
	define('CHANGEFREQ_CONTENT', 'weekly');  // Valid values are "always", "hourly", "daily", "weekly", "monthly", "yearly" and "never". 
	...
	foreach ($cat_array as $lang_array) {
		foreach ($lang_array as $cat_id => $cat_data) {
			...
			$string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(xtc_href_link(FILENAME_DEFAULT, rv_get_path($cat_data['categories_id'], $cat_data['code']) . $lang_param, NONSSL, false, SEARCH_ENGINE_FRIENDLY_URLS))), PRIORITY_CATEGORIES, iso8601_date($date), CHANGEFREQ_CATEGORIES);
	...
	$content_result = xtc_db_query(" 
		...
		FROM  
		  ...
		  " . TABLE_LANGUAGES . " l 
		WHERE 
		  cm.languages_id = l.languages_id AND 
		  cm.content_status = 1 
		ORDER by  
		  cm.content_id 
	  ");
	...
	if (xtc_db_num_rows($content_result) > 0) {
		while ($content_data = xtc_db_fetch_array($content_result)) {
			$content_array[$content_data['content_id']][$content_data['code']] = $content_data;
		}
	}
	reset($content_array);

	foreach ($content_array as $lang_array) {
		foreach ($lang_array as $content_id => $content_data) {
			$lang_param = ($content_data['code'] != DEFAULT_LANGUAGE) ? '&language=' . $content_data['code'] : '';
			$date = iso8601_date(time());
			...
			$cPath_new = 'coID=' . $content_data['content_group'] . $SEF_parameter;
			$string = sprintf(SITEMAP_ENTRY, xtc_href_link(FILENAME_CONTENT, $cPath_new . $lang_param, NONSSL, false, SEARCH_ENGINE_FRIENDLY_URLS), PRIORITY_CONTENT, iso8601_date(time()), CHANGEFREQ_CONTENT);
			$c_content_total++;
			output_entry();
		}
	}
	...
	if (xtc_db_num_rows($product_result) > 0) {
		while ($product_data = xtc_db_fetch_array($product_result)) {
			...
			$products_link = xtc_href_link(FILENAME_PRODUCT_INFO, xtc_product_link($product_data['products_id'], $product_data['products_name']) . $lang_param, 'NONSSL', false);
			$string = sprintf(SITEMAP_ENTRY, $products_link, PRIORITY_PRODUCTS, iso8601_date($date), CHANGEFREQ_PRODUCTS);
			$c_prod_total++;
			output_entry();
		}
	}
	...
	$main_content .= "<br /><br />" . $c_cat_total . " <b>Kategorien</b>, " . $c_content_total . " <b>Content</b>, " . $c_prod_total . " <b>Produkte</b> exportiert.";
	...
	if ($autogenerate && $i > 1) {
		...
		for ($ii = 1; $ii <= $i; $ii++) {
			if ($ii == 1) {
				$function_write($fp, sprintf(SITEMAPINDEX_ENTRY, SITEMAP_CATALOG . 'sitemap' . $file_extension, iso8601_date(time())));
			} else {
...
function output_entry() {
	global $string, $strlen, $c, $i, $autogenerate, $fp, $function_open, $function_close, $main_content, $strlen, $file_extension;
...
function sitemap_curl($notify_url, $mixed = array()) {
	...
	foreach ($mixed as $value) {
		if ($allow_url_fopen == 0 || function_exists('curl_exec') == true) {
			...
			$user_agent = 'Mozilla/4.0 (compatible; xtc; sitemap-submitter) commerce:SEO sitemap-submitter';
