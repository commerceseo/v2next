...
/* -----------------------------------------------------------------
 * 	$Id: xtc_format_price_order.inc.php 866 2014-03-17 12:07:35Z akausch $
...
function xtc_format_price_order($price_string, $price_special, $currency, $show_currencies = 1) {
    $currencies_value = xtc_db_fetch_array(xtDBquery("SELECT symbol_left, symbol_right, decimal_places, value FROM " . TABLE_CURRENCIES . " WHERE code = '" . $currency . "';"));
    ...
    if ($price_special == '1') {
        $currencies_value = xtc_db_fetch_array(xtDBquery("SELECT symbol_left, decimal_point, thousands_point, value FROM " . TABLE_CURRENCIES . " WHERE code = '" . $currency . "';"));
