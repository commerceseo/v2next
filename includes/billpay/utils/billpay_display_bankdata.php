 	...
 	function display_billpay_bankdata() {
		...
		if ($paymentMethod == 'billpay') {		/* INVOICE */
			...
			if (!xtc_db_num_rows($bank_data_query)) { 
				...
				$bank_data_string = sprintf(utf8_decode(MODULE_PAYMENT_BILLPAY_TEXT_INVOICE_INFO), $bank_data['invoice_reference'], substr($dueDate,6,2), substr($dueDate,4,-2), substr($dueDate,0,-4));
                ...
                if (is_numeric($bank_data['account_number']) === false) {
			...
			require_once(DIR_FS_DOCUMENT_ROOT . 'includes/modules/payment/billpaytransactioncredit.php');
