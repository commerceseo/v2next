...
function smarty_function_outerContainer($Params=array(), &$smarty) {
	...
	$top = '';
	$bottom = '';
	$links_query = xtc_db_query("SELECT id FROM boxes WHERE position = 'nav' AND status = '1';");
	$rechts_query = xtc_db_query("SELECT id FROM boxes WHERE position = 'boxen' AND status = '1';");
	$unten_query = xtc_db_query("SELECT id FROM boxes WHERE position = 'footer' AND status = '1';");
	if(PRODUCT_ID > 0 && strstr($_SERVER['REQUEST_URI'], FILENAME_SHOPPING_CART) === false && strstr($_SERVER['REQUEST_URI'], FILENAME_WISH_LIST) === false) {
		$p = xtc_db_fetch_array(xtc_db_query("SELECT products_col_top, products_col_left, products_col_right, products_col_bottom FROM ".TABLE_PRODUCTS." WHERE products_id = '".(int) PRODUCT_ID."';"));
		if($p['products_col_left'] == '1' && xtc_db_num_rows($links_query) > 0)
			$left = '_left';
		if($p['products_col_right'] == '1' && xtc_db_num_rows($rechts_query) > 0)
			$right = '_right';
		if($p['products_col_top'] == '1')
			$top = '_top';
		if($p['products_col_bottom'] == '1' && xtc_db_num_rows($unten_query) > 0)
			$bottom = '_bottom';
		...
		$c = xtc_db_fetch_array(xtc_db_query("SELECT content_col_top, content_col_left, content_col_right, content_col_bottom FROM ".TABLE_CONTENT_MANAGER." WHERE content_group = '".(int) CONTENT_ID."' AND languages_id = '".(int) $_SESSION['languages_id']."';"));
		if($c['content_col_left'] == '1' && xtc_db_num_rows($links_query) > 0)
			$left = '_left';
		if($c['content_col_right'] == '1' && xtc_db_num_rows($rechts_query) > 0)
			$right = '_right';
		if($c['content_col_top'] == '1')
			$top = '_top';
		if($c['content_col_bottom'] == '1' && xtc_db_num_rows($unten_query) > 0)
			$bottom = '_bottom';
		return '<div id="outerContainer'.$left.$right.$top.$bottom.'">';
		...
		$c5 = xtc_db_fetch_array(xtc_db_query("SELECT content_col_top, content_col_left, content_col_right, content_col_bottom FROM ".TABLE_CONTENT_MANAGER." WHERE content_group = '5' AND languages_id = '".(int) $_SESSION['languages_id']."';"));
		if($c5['content_col_left'] == '1' && xtc_db_num_rows($links_query) > 0)
			$left = '_left';
		if($c5['content_col_right'] == '1' && xtc_db_num_rows($rechts_query) > 0)
			$right = '_right';
		if($c5['content_col_top'] == '1')
			$top = '_top';
		if($c5['content_col_bottom'] == '1' && xtc_db_num_rows($unten_query) > 0)
			$bottom = '_bottom';
		return '<div id="outerContainer'.$left.$right.$top.$bottom.'">';
		...
		if($ca['categories_col_left'] == '1' && xtc_db_num_rows($links_query) > 0)
			$left = '_left';
		if($ca['categories_col_right'] == '1' && xtc_db_num_rows($rechts_query) > 0)
			$right = '_right';
		if($ca['categories_col_top'] == '1')
			$top = '_top';
		if($ca['categories_col_bottom'] == '1' && xtc_db_num_rows($unten_query) > 0)
			$bottom = '_bottom';
		return '<div id="outerContainer'.$left.$right.$top.$bottom.'">';
			...
			if(xtc_db_num_rows($links_query) > 0)
				$left = '_left';
			if(xtc_db_num_rows($rechts_query) > 0)
				$right = '_right';
			if(xtc_db_num_rows($unten_query) > 0)
				$bottom = '_bottom';
