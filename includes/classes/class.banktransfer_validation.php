...
/* -----------------------------------------------------------------
 * 	$Id: class.banktransfer_validation.php 598 2013-09-12 13:19:54Z akausch $
...
class AccountCheck {
    /* Folgende Returncodes werden uebergeben                                      */
    /*                                                                            */
    /* 0 -> Kontonummer & BLZ OK                                                  */
    /* 1 -> Kontonummer & BLZ passen nicht                                        */
    /* 2 -> Fuer diese Kontonummer ist kein Pruefziffernverfahren definiert         */
    /* 3 -> Dieses Pruefziffernverfahren ist noch nicht implementiert              */
    /* 4 -> Diese Kontonummer ist technisch nicht pruefbar                         */
    /* 5 -> BLZ nicht gefunden                                                    */
    /* 8 -> Keine BLZ uebergeben                                                   */
    /* 9 -> Keine Kontonummer uebergeben                                           */
    /* 10 -> Kein Kontoinhaber uebergeben                                          */
    /* 128 -> interner Fehler,der zeigt, das eine Methode nicht implementiert ist */
    /*                                                                            */

    var $Bankname; // Enthaelt den Namen der Bank bei der Suche nach BLZ
    var $PRZ; //Enthaelt die Pruefziffer
...
/* End of CrossSum */
    ...
    // Auffuellen der Konto-Nr. mit '0' auf 10 Stellen.
...
/* End of ExpandAccount */
    ...
    // Entfernt die fuehrenden Nullen einer Kontonummer
...
/* End of MarkB8 */
    ...
    private function MarkB9($AccountNo) {
        ...
        // Variante 1 - Zwei fuehrende Nullen
        if ((substr($AccountNo, 0, 2) == "00") And (substr($AccountNo, 2, 1) != "0")) {
            ...
            // Variante 2 - Drei fuehrende Nullen
...
/* End of MarkC0 */
    ...
    private function MarkC5($AccountNo) {
        ...
        switch ($aAccountNo['AccountNoShortLen']) {
            case 6:
                ...
                if ((int) (substr($AccountNo, 4, 1)) <= 8 && (int) (substr($AccountNo, 4, 1)) >= 1) {
    ...
    function CheckAccount($banktransfer_number, $banktransfer_blz) {
        ...
        $Result = 0;
        if ($BLZ == '' || strlen($BLZ) < 8) {
            return 8;  /* Keine BLZ uebergeben */
        }
        if ($KontoNR == '') {
            return 9;  /* Keine Kontonummer uebergeben */
        }
