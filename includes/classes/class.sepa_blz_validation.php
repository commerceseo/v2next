<?php
/* -----------------------------------------------------------------
 * 	$Id: class.sepa_blz_validation.php 846 2014-02-08 19:46:51Z akausch $
 ...
 * 	Released under the GNU General Public License
   ...
   based on:
	blz_validation.inc.php 2014-01-13 tb@gambio
	Gambio GmbH
	http://www.gambio.de
   (c) 2000-2001 The Exchange Project  (earlier name of osCommerce)
		...
		if(substr($AccountNo, 2, 1) != '9')
		{
			...
			if($Help == 1)
			{
				...
				if($Help == 1)
				{
					...
					$Help = $this->Method00($AccountNo, '000121212', 10, 10);
					if($Help == 1)
					{
						//10. Stelle 7, 8 oder 0 = ungueltig
...
// End of Mark51
	...
	protected function Mark52($AccountNo, $BIC)
	{
...
/* End of Mark52 */
	...
	protected function Mark53($AccountNo, $BIC)
	{
		$Significance = '4216379A5842';
...
/* End of Mark65 */
	...
	protected function Mark66($AccountNo)
	{
		...
		$t_timestamp = strtotime('03.03.2014 00:00:00');
		if($t_timestamp <= time())
		{
...
/* End of Mark94 */
	...
	protected function Mark95($AccountNo)
	{
		...
		{
			$Mark95 = $this->Method06($AccountNo, '432765432', FALSE, 10, 11);
			if((int)$AccountNo <= 1999999)
				$Mark95 = 4;

			if((int)$AccountNo >= 9000000 && (int)$AccountNo <= 25999999)
				$Mark95 = 4;

			if((int)$AccountNo >= 396000000 && (int)$AccountNo <= 499999999)
				$Mark95 = 4;

			if((int)$AccountNo >= 700000000 && (int)$AccountNo <= 799999999)
				$Mark95 = 4;

			if((int)$AccountNo >= 910000000 && (int)$AccountNo <= 989999999)
				$Mark95 = 4;
		}
		return $Mark95;
	}
...
/* End of MarkB5 */
	...
	protected function MarkB6($AccountNo, $BIC)
	{
...
/* End of MarkB9 */
	...
	protected function MarkC0($AccountNo, $BIC)
	{
...
/* End of MarkC0 */
	...
	protected function MarkE0($AccountNo)
	{
		...
		for($run = 0; $run < strlen($AccountNo); $run++)
		{
			$multiplicator = ($run + 1) % 2 + 1;
			$position = strlen($AccountNo) - 1 - $run;
			$crossSum = $this->CrossSum(substr($AccountNo, $position, 1) * $multiplicator) . $crossSum;
		}

		for($run = 0; $run < strlen($crossSum); $run++)
		{
			$help += substr($crossSum, $run, 1);
		}
		$help += 7;
		$help = $this->OnlyOne($help);

		if((10 - $help) == $checkDigit)
		{
			$markE0 = 0;
		}
