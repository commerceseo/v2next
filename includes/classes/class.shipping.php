...
/* -----------------------------------------------------------------
 * 	$Id: class.shipping.php 1040 2014-05-12 19:22:49Z akausch $
...
class shipping_ORIGINAL {

    var $modules;

    // class constructor
    function shipping_ORIGINAL($module = '', $admin = '') {
            ...
            for ($i = 0, $n = sizeof($include_modules); $i < $n; $i++) {
                if (xtc_in_array(str_replace('.php', '', $include_modules[$i]['file']), $unallowed_modules) != 'false') {
                    ...
                    if (in_array($_SESSION['delivery_zone'], $unallowed_zones) == true || count($unallowed_zones) == 0) {
						...
						include($modulepath . 'shipping/' . $include_modules[$i]['file']);
    ...
    function quote($method = '', $module = '') {
        global $total_weight, $shipping_weight, $shipping_quoted, $shipping_num_boxes, $total_count;
                    ...
                    for ($i = 0; $i < $size; $i++) {
                        if (array_key_exists("cost", $quotes['methods'][$i]) AND !isset($quotes['error'][$i])) {
                            $rates[] = array('id' => $quotes['id'] . '_' . $quotes['methods'][$i]['id'],
                                'title' => $quotes['module'] . (($quotes['methods'][$i]['title'] == '') ? '': ' (') . $quotes['methods'][$i]['title'] . (($quotes['methods'][$i]['title'] == '') ? '': ')'),
