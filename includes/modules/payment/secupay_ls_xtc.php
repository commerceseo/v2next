...
class secupay_ls_xtc {
     ...
     * Constructor
     */
    function secupay_ls_xtc() {
        global $order;

        $this->code = 'secupay_ls_xtc';
        $this->title = (function_exists('xtc_catalog_href_link')) ? MODULE_PAYMENT_SPLS_TEXT_TITLE . "<br/><img style='border:0;text-decoration:none;' alt='secupay logo' src='images/secupay_label_24.png'/>" : MODULE_PAYMENT_SPLS_TEXT_TITLE;
        $this->sort_order = MODULE_PAYMENT_SPLS_SORT_ORDER;
        $this->enabled = strcmp(MODULE_PAYMENT_SPLS_STATUS, 'Ja') == 0 ? true : false;
        $this->sp_log = strcmp(MODULE_PAYMENT_SPLS_LOGGING, 'Ja') == 0 ? true : false;

        //Demo Modus
        if (strcmp(MODULE_PAYMENT_SPLS_SIMULATION_MODE, "Simulation") == 0) {
            $this->demo = true;
        } else {
            $this->demo = false;
        }

        if ((int) MODULE_PAYMENT_SPLS_ORDER_STATUS_ID > 0) {
            $this->order_status_success = MODULE_PAYMENT_SPLS_ORDER_STATUS_ID;
        }
        $this->conf = get_sp_conf();
        
        $this->description = MODULE_PAYMENT_SPLS_TEXT_DESCRIPTION . "<br/>Version " . $this->conf->general->modulversion .
        ...
        $this->description.='<iframe frameborder="0" scrolling="auto" src="'.SECUPAY_INFO_IFRAME_URL.'?vertrag_id_ls=' . MODULE_PAYMENT_SECUPAY_APIKEY . '&amp;shop=' . $this->conf->general->shop . '&amp;shopversion=' . urlencode(PROJECT_VERSION) . '&amp;modulversion=' . urlencode($this->conf->general->modulversion) . '" marginwidth="0px" marginheight="0px" name="secupay_info" style="border:0px; display:block;width:98%;margin-top:3px;margin-bottom:3px;"></iframe>';

        secupay_log($this->sp_log, "secupay_ls_xtc constructor php " . PHP_VERSION);
        secupay_log($this->sp_log, "Conf: " . json_encode($this->conf));
        secupay_log($this->sp_log, "Modus: " . MODULE_PAYMENT_SPLS_SIMULATION_MODE);
        secupay_log($this->sp_log, "Sort Order: " . MODULE_PAYMENT_SPLS_SORT_ORDER);
        secupay_log($this->sp_log, "Aktiv: " . MODULE_PAYMENT_SPLS_STATUS);
        secupay_log($this->sp_log, "Shopname: " . MODULE_PAYMENT_SPLS_SHOPNAME);
        secupay_log($this->sp_log, "Order Status erfolg: " . MODULE_PAYMENT_SPLS_ORDER_STATUS_ID);
        secupay_log($this->sp_log, "Order Status accepted: " . MODULE_PAYMENT_SPLS_ORDER_STATUS_ACCEPTED_ID);
        secupay_log($this->sp_log, "Order Status denied: " . MODULE_PAYMENT_SPLS_ORDER_STATUS_DENIED_ID);
        secupay_log($this->sp_log, "Order Status issue: " . MODULE_PAYMENT_SPLS_ORDER_STATUS_ISSUE_ID);
        secupay_log($this->sp_log, "Order Status void: " . MODULE_PAYMENT_SPLS_ORDER_STATUS_VOID_ID);
        secupay_log($this->sp_log, "Order Status authorized: " . MODULE_PAYMENT_SPLS_ORDER_STATUS_AUTHORIZED_ID);
    ...
    function update_status() {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_SPLS_ZONE > 0)) {
            $check_flag = false;
            try {
                $check_query = xtc_db_query("SELECT zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '" . MODULE_PAYMENT_SPLS_ZONE . "' AND zone_country_id = '" . $order->billing['country']['id'] . "' ORDER BY zone_id");
        ...
        if (($this->enabled == true) && $this->conf->secupay_ls_xtc->delivery_differs_disable && MODULE_PAYMENT_SPLS_DELIVERY_DISABLE == "Ja") {
    ...
    function selection() {
        ...
        if (!isset($response) || $response->status != 'ok' || !in_array($this->conf->secupay_ls_xtc->payment_type, $response->data)) {
            // debit payment type is not available
            $this->enabled = false;
            return '';
        }
        $logo = "<img alt=\"secupay Lastschrift\" style=\"border:0;\" src=\"images/secupay-Lastschrift.png\" />";
        $title = $this->title;
        ...
        $selection['description'] = MODULE_PAYMENT_SECUPAY_SPLS_TEXT_INFO;
    ...
    function check() {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_SPLS_STATUS'");
    ...
    function confirmation() {
        ...
        if ($this->conf->secupay_ls_xtc->delivery_differs_disable && MODULE_PAYMENT_SPLS_DELIVERY_DISABLE == "Hinweis" && $this->_check_delivery_differs($order)) {
            $additional_messages = array('title' => MODULE_PAYMENT_SPLS_HINT, 'field' => MODULE_PAYMENT_SPLS_DELIVERY_HINT);
    ...
    function before_process() {
        ...
        if (!isset($_SESSION['sp_success'])) {
            ...
            $_mwst_bug = 0;
            if (MODULE_PAYMENT_SPLS_MWST == "Ja") {
            ...
            $store_name = MODULE_PAYMENT_SPLS_SHOPNAME;
            ...
            $_SESSION['uid'] = $uid;
            $data['url_success'] = str_replace('&amp;', '&', xtc_href_link('secupay_checkout_success.php', 'payment_error=secupay_ls_xtc&uid=' . $uid, 'SSL', true, false));
            $data['url_failure'] = xtc_href_link('secupay_checkout_failure.php', 'payment_error=secupay_ls_xtc', 'SSL', true, false);
            ...
            $data["payment_type"] = $this->conf->secupay_ls_xtc->payment_type;
            $data["payment_action"] = $this->conf->secupay_ls_xtc->payment_action;
            $data["url_push"] = xtc_href_link('secupay_status_push.php', 'payment_type=' . $this->conf->secupay_ls_xtc->payment_type, 'SSL', true, false);
    ...
    function after_process() {
        ...
        if (isset($_SESSION['sp_hash'])) {
            try {
                ...
                //write to log
                xtc_db_query("UPDATE secupay_history_ls SET ordernr = " . xtc_db_input($insert_id) . " where hash = '" . xtc_db_input($_SESSION['sp_hash']) . "';");
            } catch(Exception $e) {
        ...
        if ($is_demo) {
            $comment .= MODULE_PAYMENT_SPLS_DEMO_HINT;
        }
    ...
    function get_error() {
        ...
        $error = array('title' => MODULE_PAYMENT_SPLS_TEXT_ERROR, 'error' => stripslashes(urldecode($fehler)));
        return $error;
    }

    /**
     * Function that installs Secupay Debit payment module
     */
    function install() {
        ...
        if($this->conf->general->apikey && is_string($this->conf->general->apikey)){
            $vertrag_id_ls = trim($this->conf->general->apikey);
        }

        $modus = $this->conf->secupay_ls_xtc->modus == "live" ? "Produktivsystem" : "Simulation";
        $debug = $this->conf->secupay_ls_xtc->debug ? 'Ja' : 'Nein';
        $lw = $this->conf->secupay_ls_xtc->lieferanschrift_warnung ? 'Ja' : 'Nein';
        $modul = $this->conf->general->modul;

        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPLS_STATUS', 'Ja', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPLS_SORT_ORDER', '0',  '6', '0' , now())");
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SECUPAY_APIKEY', '{$vertrag_id_ls}',  '6', '0' , now())");
        xtc_db_query("$pref use_function, set_function, date_added) values ( 'MODULE_PAYMENT_SPLS_ZONE', '0',  '6', '0', 'xtc_get_zone_class_title', 'xtc_cfg_pull_down_zone_classes(', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPLS_ORDER_STATUS_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");

        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPLS_ORDER_STATUS_ACCEPTED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPLS_ORDER_STATUS_DENIED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPLS_ORDER_STATUS_ISSUE_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPLS_ORDER_STATUS_VOID_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("$pref set_function, use_function, date_added) values ('MODULE_PAYMENT_SPLS_ORDER_STATUS_AUTHORIZED_ID', '0',  '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPLS_SIMULATION_MODE', '$modus', '6', '0', 'xtc_cfg_select_option(array(\'Simulation\', \'Produktivsystem\'), ', now())");
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPLS_MWST', 'Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPLS_LOGGING','$debug', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SPLS_SHOPNAME', '',  '6', '0' , now())");
        //xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPLS_PREAUTH', '$aut', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");
        //xtc_db_query("$pref set_function, date_added) values ( 'MODULE_PAYMENT_SPLS_BID', 'Ja', '6', '99', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\'), ', now())");

        if ($this->conf->secupay_ls_xtc->delivery_differs_disable) {
            xtc_db_query("$pref set_function, date_added) values ('MODULE_PAYMENT_SPLS_DELIVERY_DISABLE', 'Nein', '6', '0', 'xtc_cfg_select_option(array(\'Ja\', \'Nein\', \'Hinweis\'), ', now())");
        }        
        
        xtc_db_query("$pref date_added) values ('MODULE_PAYMENT_SECUPAY_LS_XTC_ALLOWED', '', '6', '0', now())");
        xtc_db_query("DROP TABLE IF EXISTS secupay_history_ls");
        xtc_db_query("CREATE TABLE secupay_history_ls (
    ...
    function remove() {
        ...
        xtc_db_query("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key in ('" . implode("', '", $this->keys()) . "','MODULE_PAYMENT_SECUPAY_LS_XTC_ALLOWED') AND configuration_key NOT IN ('MODULE_PAYMENT_SECUPAY_APIKEY')");
     ...
     */
    function keys() {
        $keys = array('MODULE_PAYMENT_SPLS_STATUS',
            'MODULE_PAYMENT_SECUPAY_APIKEY',
            'MODULE_PAYMENT_SPLS_SORT_ORDER',
            'MODULE_PAYMENT_SPLS_SHOPNAME',
            'MODULE_PAYMENT_SPLS_SIMULATION_MODE',
            'MODULE_PAYMENT_SPLS_LOGGING',
            'MODULE_PAYMENT_SPLS_ZONE',
            'MODULE_PAYMENT_SPLS_ORDER_STATUS_ID',
            'MODULE_PAYMENT_SPLS_ORDER_STATUS_ACCEPTED_ID',
            'MODULE_PAYMENT_SPLS_ORDER_STATUS_DENIED_ID',
            'MODULE_PAYMENT_SPLS_ORDER_STATUS_ISSUE_ID',
            'MODULE_PAYMENT_SPLS_ORDER_STATUS_VOID_ID',
            'MODULE_PAYMENT_SPLS_ORDER_STATUS_AUTHORIZED_ID',
            'MODULE_PAYMENT_SPLS_MWST');
        
        if ($this->conf->secupay_ls_xtc->delivery_differs_disable) {
            $keys[] = 'MODULE_PAYMENT_SPLS_DELIVERY_DISABLE';
        }        
    ...
    function _prepare_trans_log($req_data, $ret_data, $hash, $transaction_id, $msg, $status, $amount) {
        try {
            $sql = "INSERT INTO secupay_history_ls (req_data, ret_data,payment_method,`hash`,transaction_id, msg,status,amount,action,created)" .
