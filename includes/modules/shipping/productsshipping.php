...
/* -----------------------------------------------------------------
 * 	$Id: productsshipping.php 420 2013-06-19 18:04:39Z akausch $
...
class productsshipping {
    ...
    function get_product_shipping_costs() {
        ...
        for ($i = 0; $i < sizeof($products); $i++) {
            ...
            $result = xtc_db_query("SELECT
									...
									AND
										products_id = '" . xtc_get_prid($products[$i]['id']) . "'");
            ...
            if (xtc_db_num_rows($result) > 0) {
                while (($row = xtc_db_fetch_array($result))) {
                    ...
                    $price = '';
                    if (strstr($row['costs'], ':')) {
                        $tmp = explode(';', $row['costs']);
                        for ($c = 0; $c < sizeof($tmp); $c++) {
                            $z = explode(':', $tmp[$c]);
                            if ($z[0] == $order->delivery['country']['iso_code_2']) {
                                $price = '+' . $xtPrice->xtcFormat($z[1] * $products[$i]['quantity'], true, $this->tax_class, true) . ' f&uuml;r ' . $z[0];
                    ...
                    } elseif (!empty($row['costs']) && $row['costs'] > 0) {
        ...
        if ($dest_zone == 0)
            ...
            if ($shipping == -1) {
                ...
                if ($zones_cost_full > 0)
                    $shipping_method .= $xtPrice->xtcFormat($zones_cost_full, true, $this->tax_class, true) . $products_info;
                else
    ...
    function check() {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_PRODUCTSSHIPPING_STATUS'");
    ...
    function install() {
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_SHIPPING_PRODUCTSSHIPPING_STATUS', 'True', '6', '0', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_PRODUCTSSHIPPING_ALLOWED', '', '6', '0', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_SHIPPING_PRODUCTSSHIPPING_TAX_CLASS', '0', '6', '0', 'xtc_get_tax_class_title', 'xtc_cfg_pull_down_tax_classes(', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_PRODUCTSSHIPPING_SORT_ORDER', '0', '6', '0', now())");
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $default_countries = '';
            if ($i == 1)
                $default_countries = 'DE,AT';
            xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_PRODUCTSSHIPPING_COUNTRIES_" . $i . "', '" . $default_countries . "', '6', '0', now())");
            xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_PRODUCTSSHIPPING_COST_" . $i . "', '3:8.50,7:10.50,99:20.00', '6', '0', now())");
            xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_SHIPPING_PRODUCTSSHIPPING_HANDLING_" . $i . "', '0', '6', '0', now())");
        }
    }
