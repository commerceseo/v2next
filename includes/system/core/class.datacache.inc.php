...
/* -----------------------------------------------------------------
 * 	$Id: class.datacache.inc.php 522 2013-07-24 11:44:51Z akausch $
...
class DataCache {
    ...
    function __construct() {
        if (is_object($GLOBALS['cseo_debugger'])) {
            $GLOBALS['cseo_debugger']->log('DataCache() by ' . cseo_get_env_info('REQUEST_URI'), 'DataCache');
        }
    ...
    protected function init_persistence_index_array() {
        ...
        if (is_readable($t_index_file) == false) {
            return NULL;
        }
    ...
    public function add_persistence_tag($p_key, $p_tags_array) {
        if (isset($this->v_persistence_index_array[$p_key]) == false) {
    ...
    private function write_persistence_index() {
        $t_index_file = $this->get_cache_dir() . 'persistence_index';
    ...
    public function set_data($p_key, $p_value, $p_persistent = false, $p_persistence_tags_array = false) {
        ...
        if (is_object($GLOBALS['cseo_debugger']))
            $GLOBALS['cseo_debugger']->log("new content with key " . $p_key, 'DataCache');

        if ($p_persistent) {
            $this->write_persistent_data($p_key, $p_value);
            if (is_object($GLOBALS['cseo_debugger']))
                $GLOBALS['cseo_debugger']->log("new persistent content with key " . $p_key, 'DataCache');
    ...
    public function get_data($p_key, $p_persistent = false) {
        ...
        if ($this->key_exists($p_key, $p_persistent) == false) {
            ...
            if (is_object($GLOBALS['cseo_debugger']))
                $GLOBALS['cseo_debugger']->log("cached content returned with key " . $p_key, 'DataCache');
        }
        return $t_output;
    }

    public function key_exists($p_key, $p_persistent = false) {
        if (is_object($GLOBALS['cseo_debugger']) && $GLOBALS['cseo_debugger']->is_enabled('DataCache_disable_cache'))
            return false;
        $t_output = false;

        if (isset($this->v_cache_content_array[$p_key]) == true) {
    ...
    public function get_cache_file($p_key) {
        $t_cache_file = $this->get_cache_dir() . $p_key . '-' . $this->v_cache_file_prefix . FileLog::get_secure_token() . '.pdc';
        return $t_cache_file;
    }

    public function write_persistent_data($p_key, $p_data) {
        if (is_object($GLOBALS['cseo_debugger']) && $GLOBALS['cseo_debugger']->is_enabled('DataCache_disable_persistent'))
            return false;
    ...
    public function get_persistent_data($p_key) {
        ...
        #cancel if cache file not found
        if (is_readable($t_cache_file) == false)
            return NULL;
