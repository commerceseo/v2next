...
switch ($_GET['error_message']) {
    ...
    case "2":
        ...
        break;
    case "3":
        $message = str_replace('\n', '', ERROR_WIDERRUFSRECHT_NOT_ACCEPTED);
        $messageStack->add('checkout_payment', $message);
        break;
    case "4":
        $message = str_replace('\n', '', ERROR_DATENSCHUTZ_NOT_ACCEPTED);
        $messageStack->add('checkout_payment', $message);
        break;
    case "12":
    case "32":
...
require_once(DIR_WS_CLASSES . 'class.order_total.php'); // GV Code ICW ADDED FOR CREDIT CLASS SYSTEM
...
if (xtc_count_shipping_modules() > 0) {
    ...
    $shipping_block = $module_smarty->fetch('base/module/checkout_shipping_block.html');
}
...
$module_smarty->caching = false;
$payment_block = $module_smarty->fetch('base/module/checkout_payment_block.html');
...
for ($i = 0, $n = sizeof($temp_prods); $i < $n; $i++) {
    ...
    if ($temp_prods[$i]['attributes']) {
        ...
        while (list($option, $value) = each($temp_prods[$i]['attributes'])) {
            $attributes_model .= xtc_get_attributes_model(xtc_get_prid($temp_prods[$i]['id']), $value['value'], $value['option']);
        }
        if ($attributes_model)
...
if (DISPLAY_REVOCATION_ON_CHECKOUT == 'true') {
    ...
    $smarty->assign('WD', $revocation);
    $smarty->assign('WD_LINK', $main->getContentLink(REVOCATION_ID, MORE_INFO, 'SSL'));
...
if ($order->delivery['country_id'] !== STORE_COUNTRY && CHECKOUT_SHOW_SHIPPING == 'true') {
    ...
    $shop_content_data = xtc_db_fetch_array(xtc_db_query("SELECT
                         content_text
                         FROM " . TABLE_CONTENT_MANAGER . "
                         WHERE content_group='" . CHECKOUT_SHOW_SHIPPING_ID . "'
						 " . $group_check . "
                         AND languages_id='" . $_SESSION['languages_id'] . "';"));
    $smarty->assign('SZI', $shop_content_data['content_text']);
}
