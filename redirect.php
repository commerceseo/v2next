...
/* -----------------------------------------------------------------
 * 	$Id: redirect.php 852 2014-02-22 18:16:37Z akausch $
...
switch ($_GET['action']) {
    case 'banner' :
		$goto = (int)$_GET['goto'];
        $banner_query = xtc_db_query("SELECT banners_url FROM " . TABLE_BANNERS . " WHERE banners_id = '" .  $goto . "'");
        if (xtc_db_num_rows($banner_query)) {
            $banner = xtc_db_fetch_array($banner_query);
            xtc_update_banner_click_count($goto);
