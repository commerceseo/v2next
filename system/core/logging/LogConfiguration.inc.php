...
/* --------------------------------------------------------------
  LogConfiguration.inc.php 2014-08-05 gm
  Gambio GmbH
  http://www.gambio.de
...
class LogConfiguration {
    ...
    public function load_group_preset($p_group_name = '', $p_replace_group_name = true, $p_suppress_db = false) {
        ...
        if ($p_suppress_db == false && $this->load_db_configuration($t_group_name) && mysql_num_rows($t_result) > 0) {
            return true;
        }
    ...
    public function is_active_output_type($p_level, $p_output_type) {
        return isset($this->configuration_array[$p_level]) && isset($this->configuration_array[$p_level][$p_output_type]) && isset($this->configuration_array[$p_level][$p_output_type]['output']) && $this->configuration_array[$p_level][$p_output_type]['output'];
    }
    ...
    public function get($p_level, $p_output_type, $p_output) {
        return isset($this->configuration_array[$p_level]) && isset($this->configuration_array[$p_level][$p_output_type]) && isset($this->configuration_array[$p_level][$p_output_type][$p_output]) && $this->configuration_array[$p_level][$p_output_type][$p_output];
    }
    ...
    protected function is_shop_environment() {
        return defined('STORE_NAME') && function_exists('xtc_db_query') && is_resource($GLOBALS['db_link']);
    }
    ...
    public function get_configuration_html_form() {
        ...
        foreach ($this->level_array as $t_level) {
            ...
            foreach ($this->output_type_array as $t_output_type) {
                ...
                foreach ($this->output_array as $t_output) {
                    $t_value = isset($this->configuration_array[$t_level]) && isset($this->configuration_array[$t_level][$t_output_type]) && isset($this->configuration_array[$t_level][$t_output_type][$t_output]) && $this->configuration_array[$t_level][$t_output_type][$t_output];
                    $t_checked = '';
                    if ($t_value) {
        ...
        $t_form .= '<script type="text/javascript">
