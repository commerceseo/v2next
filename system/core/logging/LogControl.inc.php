...
class LogControl {
    ...
    public function write_sql_log($p_query = '', $p_type = 'QUERY', $p_level = 'notice', $p_error_code = 0, $p_error_description = '', $p_output_type_array = array()) {
        if (($this->enabled == false || defined('APPLICATION_RUN_MODE') == false || defined('LOG_SQL_BACKEND') == false || defined('LOG_SQL_FRONTEND') == false || APPLICATION_RUN_MODE == 'frontend' && LOG_SQL_FRONTEND == 'false' || APPLICATION_RUN_MODE == 'backend' && LOG_SQL_BACKEND == 'false') && (is_object($GLOBALS['coo_debugger']) == false || $GLOBALS['coo_debugger']->is_enabled('log_sql_queries') == false) && $p_level != 'error') {
            // log deactivated. cancel write.
            return true;
        }
    ...
    public function write_time_log($p_message = '', $p_output_type_array = array()) {
        ...
        if (defined('STORE_PAGE_PARSE_TIME') && STORE_PAGE_PARSE_TIME == 'true' || isset($GLOBALS['coo_debugger']) && $GLOBALS['coo_debugger']->is_enabled('StopWatch')) {
            $coo_configuration->set('notice', 'file', 'output');
        }
        if (defined('DISPLAY_PAGE_PARSE_TIME') && DISPLAY_PAGE_PARSE_TIME == 'true' || isset($GLOBALS['coo_debugger']) && $GLOBALS['coo_debugger']->is_enabled('PageParseTime')) {
...
//		$this->write_stack(array('time_log'));
    ...
    protected function write_to_file($p_message, $p_file_name, $p_file_extension = '') {
        ...
        if (@filesize($this->get_file_path($p_file_name, $p_file_extension)) >= $this->max_filesize &&
