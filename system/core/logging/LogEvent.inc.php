...
class LogEvent {
    ...
    public function __construct($p_message, $p_group = '', $p_log_file_name = 'debug', $p_level = 'notice', $p_level_type = 'USER NOTICE', $p_error_code = E_USER_NOTICE, $p_additional_info = '', $p_coo_configuration = null) {
    ...
    protected function generate_backtrace_output($p_output_type = 'text', $p_output_mode = 'screen') {
        ...
        if ($this->get_specific_output($p_output_mode, 'filepath') && $this->get_specific_output($p_output_mode, 'backtrace')) {
            ...
            foreach ($this->backtrace_array as $t_backtrace_array) {
                ...
                if ($this->get_specific_output($p_output_type, 'code_snippet') || $this->get_specific_output($p_output_type, 'class_data') || $this->get_specific_output($p_output_type, 'function_data')) {
    ...
    protected function generate_class_data_output($p_output_type = 'text', $p_output_mode = 'screen', $p_backtrace_array = array()) {
        ...
        if ($this->get_specific_output($p_output_mode, 'class_data') && isset($p_backtrace_array['class']) && $p_backtrace_array['class'] != '' && $this->configuration->is_excluded_class($p_backtrace_array['class']) == false) {
            $t_div_id = $this->generate_div_id();
    ...
    protected function generate_function_data_output($p_output_type = 'text', $p_output_mode = 'screen', $p_backtrace_array = array()) {
        ...
        if ($this->get_specific_output($p_output_mode, 'function_data') && isset($p_backtrace_array['function']) && $p_backtrace_array['function'] != '' && (isset($p_backtrace_array['class']) == false || $p_backtrace_array['class'] == '' || isset($p_backtrace_array['class']) && $p_backtrace_array['class'] != '' && $this->configuration->is_excluded_class($p_backtrace_array['class']) == false)) {
            $t_div_id = $this->generate_div_id();
    ...
    protected function is_excluded_call($p_call_array) {
        return isset($p_call_array['file']) && $this->configuration->is_excluded_file($p_call_array['file']) && isset($p_call_array['class']) && $this->configuration->is_excluded_class($p_call_array['class']) || isset($p_call_array['file']) == false && isset($p_call_array['class']) && $this->configuration->is_excluded_class($p_call_array['class']) || isset($p_call_array['file']) && isset($p_call_array['class']) == false && $this->configuration->is_excluded_file($p_call_array['file']);
    }
