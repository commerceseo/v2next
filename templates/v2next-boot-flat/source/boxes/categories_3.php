<?php
/*-----------------------------------------------------------------
* 	$Id: categories_3.php 371 2013-06-10 12:44:12Z akausch $
...
if (!CacheCheck() && !FORCE_CACHE) {
	...
	if (CAT_NAV_AJAX == 'true') {
		...
		$cache_id = $_SESSION['language'].$_SESSION['customers_status']['customers_status_id'].'section'.$cPath.'cat_3';
	}
}
...
if(!$box_smarty->isCached(CURRENT_TEMPLATE.'/boxes/box.html', $cache_id) || !$cache){
	...
	function gunnartCategoriesThree($CatID=0,$Level=1) {
		...
		while($dbQueryResult = xtc_db_fetch_array($dbQuery,true)) {
			...
			if(($ProdsInCat != 0 && $CatConfig['HideEmpty'] == true) || ($CatConfig['HideEmpty'] == false)) {
				...
				if(($Level < $CatConfig['MinLevel'] || $Current) && ($Level < $CatConfig['MaxLevel'] || !$CatConfig['MaxLevel'])) {
					$Return	.= 	gunnartCategoriesThree($dbQueryResult['categories_id'],$Level+1); // <-- Rekursion!
				}
				$Return 	.=	'</li>';
			}
		}
	...
	$box_smarty->assign('box_name', getBoxName('categories_3'));
	$box_smarty->assign('box_class_name', getBoxCSSName('categories_3'));
	$box_smarty->assign('BOX_CONTENT',gunnartCategoriesThree());
}
if (!$cache) {
