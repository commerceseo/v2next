<?php
/*-----------------------------------------------------------------
* 	$Id: categories_nav_5.php 371 2013-06-10 12:44:12Z akausch $
...
if(!$box_smarty->isCached(CURRENT_TEMPLATE.'/boxes/box_categories_nav_5.html', $cache_id) || !$cache){
	...
	function gunnartCategoriesNav_5($CatID=0,$Level=1) {
		...
		while($dbQueryResult = xtc_db_fetch_array($dbQuery,true)) {
			...
			if(($ProdsInCat != 0 && $CatConfig['HideEmpty'] == true) || ($CatConfig['HideEmpty'] == false)) {
				...
				if(($Level < $CatConfig['MinLevel'] || $Current) && ($Level < $CatConfig['MaxLevel'] || !$CatConfig['MaxLevel'])) {
					$Return	.= 	gunnartCategoriesNav_5($dbQueryResult['categories_id'],$Level+1); // <-- Rekursion!
				}
				$Return 	.=	'</li>';
			}
		}
	...
	$box_smarty->assign('box_name', getBoxName('categories_nav_5'));
	$box_smarty->assign('box_class_name', getBoxCSSName('categories_nav_5'));
	$box_smarty->assign('BOX_NAME', getBoxTitle('categories_nav_5'));
	$box_smarty->assign('BOX_CONTENT',gunnartCategoriesNav_5());
}

if (!$cache) {
	$box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_categories_nav_5.html');
} else {
	$box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_categories_nav_5.html', $cache_id);
}
