<?php
/*-----------------------------------------------------------------
* 	$Id: categories_nav_6.php 371 2013-06-10 12:44:12Z akausch $
...
if (!CacheCheck() && !FORCE_CACHE) {
	...
	if (CAT_NAV_AJAX == 'true') {
		$cache_id = $_SESSION['language'].$_SESSION['customers_status']['customers_status_id'].'categories_nav_6';
	} else {
		$cache_id = $_SESSION['language'].$_SESSION['customers_status']['customers_status_id'].$cPath;
	}
}

if(!$box_smarty->isCached(CURRENT_TEMPLATE.'/boxes/box_categories_nav_6.html', $cache_id) || !$cache){
	...
	function gunnartCategoriesNav_6($CatID=0,$Level=1) {
		...
		while($dbQueryResult = xtc_db_fetch_array($dbQuery,true)) {
			// echo $dbQueryResult['categories_name'];
			$Current = false;
			...
			if(($ProdsInCat != 0 && $CatConfig['HideEmpty'] == true) || ($CatConfig['HideEmpty'] == false)) {
				...
				if(($Level < $CatConfig['MinLevel'] || $Current) && ($Level < $CatConfig['MaxLevel'] || !$CatConfig['MaxLevel'])) {
					$Return	.= 	gunnartCategoriesNav_6($dbQueryResult['categories_id'],$Level+1); // <-- Rekursion!
				}
				$Return 	.=	'</li>';
			}
		}
	...
	$box_smarty->assign('box_name', getBoxName('categories_nav_6'));
	$box_smarty->assign('box_class_name', getBoxCSSName('categories_nav_6'));
	$box_smarty->assign('BOX_NAME', getBoxTitle('categories_nav_6'));
	$box_smarty->assign('BOX_CONTENT',gunnartCategoriesNav_6());
}

if (!$cache) {
	$box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_categories_nav_6.html');
} else {
	$box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_categories_nav_6.html', $cache_id);
}
