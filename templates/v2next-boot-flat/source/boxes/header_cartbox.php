<?php
/*-----------------------------------------------------------------
* 	$Id: shopping_cart.php 371 2013-06-10 12:44:12Z akausch $
...
if ($_SESSION['customers_status']['customers_status_show_price'] == 1) {
	...
	if ($_SESSION['cart']->count_contents() > 0) {
		...
		for ($i = 0, $n = sizeof($products); $i < $n; $i ++) {
			if ($products[$i]['image'] != '') {
				$image = xtc_image(DIR_WS_MINI_IMAGES.$products[$i]['image'],$products[$i]['name']);
			} else {
				$image =  xtc_image(DIR_WS_MINI_IMAGES.'no_img.jpg',$products[$i]['name']);
			}
			$qty += $products[$i]['quantity'];
			$products_in_cart[] = array ('QTY' => $products[$i]['quantity'], 
										'IMAGE' => $image,
										'SINGLE_PRICE' => $xtPrice->xtcFormat($products[$i]['p_single_price'], true),
	...
	if ($_SESSION['cart']->count_contents() > 0) {
		$box_smarty->assign('cart_count',$_SESSION['cart']->count_contents());
	}

	if ($_SESSION['wishList']->count_contents() > 0) {
		$box_smarty->assign('wish_list_count',$_SESSION['wishList']->count_contents());
	}
	...
	$box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_header_cartbox.html');
}
$smarty->assign('BOX_header_cart', $box_content);